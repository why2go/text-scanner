...
func (t *ACTrie) Put(rkey []rune) {
	...
	if !curNode.isTerminal {
		...
		curNode.suffixLengths.add(len(rkey))
	}
}
...
func (t *ACTrie) FindMatches(rtext []rune) []scanner.Match {
	...
	for i := range rtext {
		...
		state = state.next[cp]
		if state != nil {
			for _, l := range state.suffixLengths.getAll() {
		...
		for _, nextNode := range curNode.next {
			...
			for {
				...
				if ok {
					...
					for _, v := range nextFailure.suffixLengths.getAll() {
						nextNode.suffixLengths.add(v)
					}
					break
				} else {
...
type node struct {
	...
	next          map[rune]*node
	failure       *node
	suffixLengths *lengthSet // 用于找到matches
}
...
func (ls *lengthSet) getAll() []int {
	keys := make([]int, 0, len(ls.m))
	for k := range ls.m {
		keys = append(keys, k)
	}
	return keys
